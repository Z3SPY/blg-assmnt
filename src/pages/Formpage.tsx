...
You can add more images:`,
    ...
    // ==================================
    // CHECK FOR INSTANCE OF EDIT MODE
    useEffect(() => {
        const fetchBlogData = async () => {
            if (!id) { setIsUpdating(false); return; } else {  setIsUpdating(true)}// not for update

            try {
                setFileIsLoading(true);
                const data = blogRepository.getBlogById(id);
                if (data) {
                    setBlogTitle((await data).title);
                    setCoverPath((await data).cover_path);

                    editor?.commands.setContent((await data).content);
                }
            } catch (error) {
                console.log(error)
            } finally {
    ...
    const handleBlogUpload = async (markdown: string) => {
      ...
      try {
        ...
        const file : File | null = file2Upload.current;
        ...
        if (!isUpdating) {
            ...
            await blogRepository.updateBlog( id,newBlog,file);
      ...
      <div className='h-screen overflow-y-auto overflow-x-hidden '>
          ...
          <div className="flex gap-2">
              ...
              <Button onClick={handleSave} className='transition-all hover:translate-y-[5px] hover:bg-neutral-600'>
               {!isUpdating ? "Save Your Work" : "Update Blog"}
              </Button>
          </div>
